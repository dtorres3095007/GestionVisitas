Dropzone.options.Subir = {
    url: "../model/Parametros.php?subirarchivo=si", //se especifica cuando el form no tiene el aributo action, por de fault toma la url del action en el formulario
    ...
    acceptedFiles: "image/*,.js,.css,.php", //EJEMPLO PARA PDF WORD ETC ,application/pdf,.psd,.DOCX",
       ...
       this.on("removedfile", function(file) { 
        ...
        $("#montar").click(function() {
            if ($("#Ruta").val() != "") {
               ...
               MensajeConClase("Archivos Cargados Al servidor", ".error");
                }else{
                 MensajeConClase("Seleccione Archivos", ".error");   
                }
                
            } else {
                MensajeConClase("Seleccione Ruta", ".error");
            }
        });
